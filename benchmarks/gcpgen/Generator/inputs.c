...
long *order;
{
	printf("The order of a graph is the number of vertices in it.\nPlease input the order of the graph: ");
	scanf("%ld",order);
	printf("Order = %ld\n",*order);
}
...
double *probability;
{
	printf("This graph class uses probability to assign edges\nPlease input a floating point number between 0 and 1: ");
...
{
	printf("Partitional variation: If set to 0, each partition element will be\nas nearly as possible of equal size. If set larger, then elements\nwill vary in size in a uniform manner from ((order/part)-pv) to\n((order/part)+pv).\n\Please input Partitional Variation in the range 0 to %ld: ",order/partition-1);
	scanf("%ld",vari);
			...
			double *alpha, int *addflag2, double *gamma)
{
	...
	printf("increases clustering.\n");
        printf("-------------------------------------\n");
        printf("\nChange weights by (0) Multiplying or (1) Adding to old weight?  ");
        scanf("%d",addflag1);
	if( *addflag1 ) {
		printf("Adding\n");
	} else {
