...
#define N_CLAUSE 300 //9
#define N_LITERAL 3
const int MAX_N_STEP = 1000000;
const int EPSILON = 687194767; //fixed = 0.32 (=0.32x2^31)
#define MAX_CONTRA 200000
...
int main(int argc, char* argv[]) {
    char filename[128]="flat30-100.cnf";
    char logfile[128];
...
int amoebasat(char s[N_VARIABLE+100]){
    max_continuous = 0;
    for(int i = 1; i <= N_VARIABLE; i++) x_continuous[i] = 0;
...
void update_x(){
    ...
    for(i=1; i<=N_VARIABLE; i++){
        ...
        if(x_fixed[i] < 1){
            if(x[i] == t) x_continuous[i]++;
            else{
                count++;
                x_continuous[i] = 1;
            }
        }
        if(x_fixed[i] < 1 && max_continuous < x_continuous[i]) max_continuous = x_continuous[i];
        //
        //printf("x[%d]=%d \t", i, x[i]);
    }
}
void update_Y(){
    ////
    double back[N_VARIABLE+1][2] = {};// LargeXをデクリメント
    for(int i = 0; i < N_CLAUSE; i++){
        int a = abs(f[i][0]);
        int b = abs(f[i][1]);
        int v = x_continuous[abs(f[i][0])];
        if(f[i][1] == 0 || f[i][2] != 0) continue;
        if(x_fixed[a] == 1 || x_fixed[b] == 1) continue;
        double p = 0;
        if(v > 8000) p = 0.4;
        //if(v > 5000) p = 0.5;
        //else if(v > 3000) p = 0.4;
        if(L[a][1] == 1){ back[a][0] = p; back[a][1] = 0; }
        else{ back[a][0] = 0; back[a][1] = p; }
        if(L[b][1] == 1){ back[b][0] = p; back[b][1] = 0; }
        else{ back[b][0] = 0; back[b][1] = p; }
    }
    //
    for(int i = 1;i <= N_VARIABLE; i++){
        for(int j = 0; j < 2; j++){
        	//// ここが改良対象
            double epsilon;
            if(back[i][j] > 0) epsilon = back[i][j];
            else{
                if(delta > 2000) epsilon = 0.32;
                else if(delta > 1500) epsilon = 0.3;
                else if(delta > 1000) epsilon = 0.28;
                else if(delta > 500) epsilon = 0.25;
                else epsilon = 0.25;
                /*if(delta > 1000) epsilon = 0.32;
                else if(delta > 700) epsilon = 0.28;
                else if(delta > 500) epsilon = 0.25;
                else epsilon = 0.25;*/
            }
        	long long sub = (1LL<<31) - epsilon * (1LL<<31) - Z[i][j];
            //
