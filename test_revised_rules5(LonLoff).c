<<<<<<< HEAD:test_revised_rules5(LonLoff).c
#include<stdio.h>
//#include<stdlib.h>
#include<string.h>
#include<time.h>
#define N_VARIABLE 300
#define N_CLAUSE 1117 //9
#define N_LITERAL 3
...
int L_intra[N_VARIABLE+1][2];
int L_inter_on[N_VARIABLE+1][2];
int L_inter_off[N_VARIABLE+1][2];
int L_contra[N_VARIABLE+1][2];
...
int main() {
    ...
    char filename[128]="flat100-100.cnf";
    char logfile[128];
...
void update_L_intra(){
    int i,j;
    for(i=1;i<=N_VARIABLE;i++){
    	L_intra[i][0] = (LargeX[i][1]>0);
    	L_intra[i][1] = (LargeX[i][0]>0);
    }
}
void update_L_inter(int inter[3*N_CLAUSE][6]){
    int i;
    for(i=1;i<=N_VARIABLE;i++){
        L_inter_on[i][0] = 0; L_inter_on[i][1] = 0;
        L_inter_off[i][0] = 0; L_inter_off[i][1] = 0;
    }
    for(i=0;i<3*N_CLAUSE;i++){
        ...
        //INTER
        L_inter_on[ inter[i][4] ][ inter[i][5] ] = L_inter_on[ inter[i][4] ][ inter[i][5] ] | inter1;
        //COLLAPSE
        L_inter_off[ inter[i][4] ][ 1-inter[i][5] ] = L_inter_off[ inter[i][4] ][ 1-inter[i][5] ] | inter1;
        //printf("ok %d ", L[ inter[i][0]-1 ][ inter[i][1] ]);
    }
}
void update_L_contra(int size_contra, int contra[size_contra][8]){
    int i, j;
    for(i=1;i<=N_VARIABLE+1;i++){
        L_contra[i][0]=0;
        L_contra[i][1]=0;
    }
    for(i=0;i<size_contra;i++){
        ...
        /*if(contra1){
            L_contra[ contra[i][0] ][ contra[i][1] ] = 1;
            L_contra[ contra[i][2] ][ contra[i][3] ] = 1;
            L_contra[ contra[i][4] ][ contra[i][5] ] = 1;
            L_contra[ contra[i][6] ][ contra[i][7] ] = 1;
        }*/
        //loosen representation of CONTRA - should combine with CONFLICT, not good with hypercontra
        L_contra[ contra[i][0] ][ contra[i][1] ] = L_contra[ contra[i][0] ][ contra[i][1] ] | contra1;
        L_contra[ contra[i][2] ][ contra[i][3] ] = L_contra[ contra[i][2] ][ contra[i][3] ] | contra1;
        L_contra[ contra[i][4] ][ contra[i][5] ] = L_contra[ contra[i][4] ][ contra[i][5] ] | contra1;
        L_contra[ contra[i][6] ][ contra[i][7] ] = L_contra[ contra[i][6] ][ contra[i][7] ] | contra1;
...
void update_L(){
    ...
    for(i=1;i<=N_VARIABLE;i++){
        for(j=0;j<=1;j++){
            L[i][j] = (L_intra[i][j] | L_inter_on[i][j] | L_contra[i][j]) & (!L_inter_off[i][j]);
        }
    }
}
...
>>>>>>> 31afecb2918f4e984f1d28b18a7307f312737425:test_revised_rules.c
